...
function downloadImageAndStoreLocal(bodyInnerText, topic) {
...
function createTopicFromTitle(title, group) {
...
function retrieveCached(topic) {
    ...
    Windows.Storage.ApplicationData.current.localFolder.getFileAsync(localImageSrc).then(function (file) {
...
function downloadAndCacheLocally(topic) {
    ...
    var title = topic.title;
    var url = "http://en.wikipedia.org/w/index.php?title=" + encodeURI(title);
    var htmlContentPromise = WinJS.xhr({ url: url }).then(function (result) {
...
function findImage(bodyHtml) {
    ...
    var imgs = Array.prototype.slice.call(dummyDiv.getElementsByTagName('img'), 0);
